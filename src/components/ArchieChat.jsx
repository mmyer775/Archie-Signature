...
export function ArchieChat({ user }) {
    ...
    async function loadContext() {
      ...
      try {
        const [orders, numbers, ddData, struggles] = await Promise.all([
          CONFIG.sheets.orders    ? fetchOrders(CONFIG.sheets.orders, user.accessToken, user.name)       : Promise.resolve([]),
          CONFIG.sheets.numbers   ? fetchNumbers(CONFIG.sheets.numbers, user.accessToken, user.name)     : Promise.resolve([]),
          CONFIG.sheets.master    ? fetchDDData(CONFIG.sheets.master, user.accessToken, user.name, CONFIG.officeLegalName) : Promise.resolve([]),
          CONFIG.sheets.struggles ? fetchStruggles(CONFIG.sheets.struggles, user.accessToken, user.name) : Promise.resolve([]),
        ]);
  ...
  const logToStruggles = useCallback(async (question, archieResponse, topic) => {
    ...
    if (!CONFIG.sheets.struggles || !user?.accessToken) return;
    ...
    try {
      ...
      await logStruggle(CONFIG.sheets.struggles, user.accessToken, {
        repName:        user.name,
        topic,
